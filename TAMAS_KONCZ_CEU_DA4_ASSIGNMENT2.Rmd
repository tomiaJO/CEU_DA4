...
```{r, echo = FALSE}
fun_roc_create <- function(prediction_probs, truth) {
...
```{r}
fun_roc_create(glm_prediction_probs, test_truth)
```
...
```{r}
glm_AUC <- fun_calc_auc(glmnet_prediction_probs, test_truth)
print(glm_AUC)
```
...
```{r}
fun_roc_create(glmnet_prediction_probs, test_truth)
```
...
```{r}
AUC <- fun_calc_auc(rpart_prediction_probs, test_truth)
print(AUC)
```
...
```{r}
fun_roc_create(rpart_prediction_probs, test_truth)
```
