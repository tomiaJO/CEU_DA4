...
However, here the goal is to create the best predictive model, with the great-ease that this being a University project, there is no need to create an explainable model for stakeholders.  
...
Rather, we are using LASSO- and Ridge regularization, and let the lamdba parameter be optimized by glmnet's grid-search algorithm (below range for lambda search was pre-selected based on some trial runs).  
...
The third method is a single tree, which has a nice advantage compared to previous method, as it is able to pick-up non-linearities without much interaction, simply by creating multiple splitting-nodes of the same variable (similarly it goes for interactions between variables).  
...
Is this a good think? In  my view, predicting probabilities should be a conservative venture.  
  
If I'm a lender, I'm certainly missing out on revenue opportunities if I'm overly risk-averse. However, our loss function should not be symmetrical, neither linear in this case - larger losses are almost certainly exponentially more painful than smaller ones, while by virtue of the business of lending (if we predict probabilities for that purpose), gains are limited.  

Hence, my choice for the best model is model #4. Measured by AUC, it's a draw between this one and a signle tree.  
