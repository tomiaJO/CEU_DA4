...
model_2_kensington_chelsea <- train(log_price ~ n_accommodates + n_beds + n_bathrooms +
                                          ...
                                          method = "lm",
                                          trControl = fit_control)
predictions <- predict(model_2_kensington_chelsea, kensington_chelsea_test)

RMSE(predictions, kensington_chelsea_test$log_price)
length(kensington_chelsea_train$log_price)
model_2_kensington_chelsea
model_2_kensington_chelsea$results[["RMSE"]]
```

```{r}
#checking for multicollinearity
findLinearCombos(data.matrix(london[, .SD, .SDcols = sapply(london, is.numeric)]))
...
set.seed(93) #for reproducibility
model_4_london <- train(log_price ~ . -price -d_total -f_neighbourhood_cleansed -neighbourhood_cleansed -d_washerdryer -d_freeparkingonstreet, 
                    data = kensington_chelsea_train, 
                    method = "lm",
