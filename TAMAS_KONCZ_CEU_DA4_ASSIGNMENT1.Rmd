...
ggplot(data= logrmse.table) +
  ...
  labs(y = "RMSE of log predictions", x = "Model", title= "RMSE values after cross-validation, training sample") +
...
As the first step, we'll make predictions on the trainings set with all our models. Reference for these predictions can be found here: https://stackoverflow.com/questions/31629673/determine-predictors-in-a-model-generated-with-caret-package.
```{r}
model_1_london_log_pred <- predict(model_1_london)
n <- london_train[,.N]
k <- length(predictors(model_2_london))
model_1_london_s2 <- RMSE(model_1_london_log_pred, 
                          london_train$log_price) / (n-k)
